...
pub fn run(input: String) {
    ...
    for x in 0..=x_max {
        if galaxies.iter().all(|g| g.0 != x) {
            expand_galaxies(&mut new_galaxies, x + expanded, true);
            expanded += 1;
        }
    }
    expanded = 0;
    for y in 0..=y_max {
        if galaxies.iter().all(|g| g.1 != y) {
            expand_galaxies(&mut new_galaxies, y + expanded, false);
            expanded += 1;
        }
    }
    let mut total = 0;
...
fn expand_galaxies(galaxies: &mut Vec<(usize, usize)>, target: usize, row: bool) {
